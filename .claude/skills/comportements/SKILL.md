...
|---|---|---|---|
| `fight` | 200 | `fightSystem.js` (+ frappes dans `combatSystem.js`, modulées par l'`equipment` : arme = bonus de dégâts, armure = atténuation `max(1, dégâts − défense)`) | hostile qui **peut m'atteindre** (`threatField ≤ 6`, distance de cheminement) ET courageux (`health/max >= courageThreshold`, voir plus bas) |
| `flee` | 200 | `fleeSystem.js` (fuit vers le refuge sûr le plus proche via la carte de menace `threatField` — case que les hostiles ne peuvent atteindre, typiquement derrière une porte `blocksHostiles` ; repli sur l'éloignement glouton `stepAway` si aucun refuge) | hostile qui **peut m'atteindre** (`threatField ≤ 6`) ET pas courageux |
| `hold` | 20 | `fleeSystem.js` (même repli vers l'abri que `flee`, mais **sans** panique : ni marqueur `fleeing`, ni événement, ni malus de moral) | hostile proche **à vol d'oiseau** (≤ 6 cases) mais qui **ne peut pas m'atteindre** (`threatField = Infinity` : muré ou derrière une porte). On se met à l'abri puis on tient sa position au lieu d'errer vers lui. Un vrai besoin (faim, soif…) reprend le dessus |
...
- **Danger réel = distance de cheminement (`threatField`), alerte = proximité à vol d'oiseau** : `fight`/`flee` ne se déclenchent que si un hostile **peut m'atteindre** (`threatField.js` : petit système enregistré juste avant l'arbitre, recalcule chaque tick une carte BFS de distance-menace depuis les hostiles, `Infinity` = inatteignable ; partagé par l'arbitre et `fleeSystem`, une seule fois par tick). Un hostile **proche mais bloqué** (muré, ou derrière une porte `blocksHostiles`) ne provoque donc **ni panique ni charge impossible** — il déclenche l'état `hold` (aux aguets) : le nain se met à l'abri et tient sa position sans errer vers lui. `hold` réutilise le repli de `fleeSystem` (donc il traverse bien une porte pour se figer du bon côté), mais sans le marqueur/événement de panique.
...
- **Relations** (`socializeSystem.js`) : composant data pur `relationships {affinities: {entityId: score}}`. On socialise **de préférence avec ses amis à portée** (`pickCompanion` : parmi les nains dans un rayon `COMPANION_RANGE`, score `affinité − distance` → des cliques se forment ; hors de portée, repli sur le plus proche pour éviter les traversées de carte et la poursuite d'un ami lointain qui bouge) ; côte à côte l'affinité monte (palier ami à +30, `dwarf.befriended`), une rixe (`provoked`) la fait chuter (palier rival à −30, `dwarf.fell-out`). L'affinité est écrite dans les deux sens ; l'UI purge à la lecture les liens vers un nain disparu (auto-réparable, jamais de nettoyage à la mort). Seul `socializeSystem.js` lit/écrit `relationships`.
- **Personnalité** (`personality {sociability, temper, bravery}`, tirée au spawn par `assignPersonality`, 0,5 = neutre) : `sociability` accélère le besoin social (`social.rate` ajusté au spawn) et la formation des liens (`BOND_STEP`) ; `temper` module la vitesse des rancunes (`RIVAL_STEP`) et **conditionne la bagarre d'ivrogne** (arbitre : seul un ivrogne `temper ≥ 0,5` cherche la rixe) — un ivrogne belliqueux cible **de préférence un rival proche** (`brawlSystem.pickRival`), à défaut le plus proche ; `bravery` module le seuil de fuite (voir la bravoure ci-dessous).
- **Bravoure** (`arbiterSystem.courageThreshold`) : `fight`/`flee` comparent le ratio de santé à un **seuil de fuite effectif**, pas au `combat.courage` brut. La santé reste dominante ; le seuil part de `combat.courage` (0,5) et s'abaisse (= plus brave, tient plus bas) selon trois leviers cumulés, planché à `COURAGE_FLOOR` (0,15) et jamais plafonné (un `courage` volontairement énorme dans les données fait toujours fuir) : le **moral** au-dessus/en dessous de sa `baseline` (`MORALE_COURAGE_INFLUENCE`), le trait **`bravery`** de la personnalité (`PERSONALITY_COURAGE_INFLUENCE`), et l'**effet de groupe** — chaque allié portant le marqueur `fighting` à portée (`ALLY_COURAGE_RANGE`) rassure, plafonné à `ALLY_COURAGE_CAP`. L'effet de groupe lit le marqueur du **tick précédent** (pas de cascade intra-tick). Conséquence : un nain « reprend courage » en étant soigné (santé), remonté au moral, ou entouré d'alliés au combat.
