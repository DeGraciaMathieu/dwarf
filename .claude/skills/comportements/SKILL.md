...
- **Boire** (`drinkSystem.js`) : préfère une **bière** atteignable (composant `drink`, détruite en buvant, événement `dwarf.drank-beer`, +15 de moral), sinon la berge atteignable la plus proche (tri par distance + `findPath` de vérification — jamais la plus proche aveuglément, ça gèle le nain). Une berge = case praticable touchant l'eau ; un pont compte. La cible bière est revalidée chaque tick — un hauler ou un autre buveur peut l'avoir déplacée/détruite. **En hiver** (`seasonSystem.isWinter`) les berges sont gelées : seule la bière est proposée, et sans bière atteignable la crise d'isolement (`DWARF_ISOLATED_FROM_WATER`) se déclenche naturellement.
