...
| `tiles.json` | `floor`, `wall`, `ore`, `tree`, `door`, `water`, `ice` (eau gelée d'hiver, posée par `freezeSystem`), `bridge` — `{glyph, color, walkable, blocksHostiles?}` | `terrain.js`, `renderer.js` |
