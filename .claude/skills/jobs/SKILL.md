...
| `haul` | `haulSystem.js` | auto-posté avec capacité bornée par pools de zones, deux phases, réservations auto-réparées (`pruneReservations`), lâcher générique (`dropOrphanedItems`) ; zones de stockage typées via `kind` (`food` : composants `food`/`drink`, `materials` : `buildMaterial`, `pantry` : `food`/`drink` mais conserve les périssables, sans étiquette : tout) — un objet peut convenir à plusieurs types ; la zone spécifique prime sur la générale, et le garde-manger est préféré pour un `perishable` |
